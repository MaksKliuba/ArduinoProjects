...
void loop()
{
  ...
  if (voice.length() > 0)
  {
    ...
    if(voice.indexOf("light on") >= 0 || voice.indexOf("on the light") >= 0 || voice == "lOn")
    {
    ...
    else if(voice.indexOf("light off") >= 0 || voice.indexOf("off the light") >= 0 || voice == "lOff")
    {
    ...
    else if(voice.indexOf("bumper on") >= 0 || voice == "irOn")
    {
    ...
    else if(voice.indexOf("bumper off") >= 0 || voice == "irOff")
    {
  ...
  if((millis() - timerMove >= AFTER_MOVE_TIME && !moving) || irOn)
  {
      //if(!moving)
      {
...
void Settings()
{
  ...
  if(p.z > minP && p.z < maxP)
  {
    ...
    else if(p.x >= 105 && p.x <= 360 && p.y >= 270 && p.y <= 300)
    {
      speed = p.x - 105; 
      
      speedCheck = false;
    }
    else if(p.x >= 105 && p.x <= 405 && p.y >= 200 && p.y <= 230)
    {
      loudness = (p.x - 96)/10;
...
void updateSettingsData()
{
  ...
  sendData("0:" + (String)loudness);
  
  sendData("1:" + (String)speed);
}

void printSpeed(int speed)
{
  printTFT(30, 270, 2, "SPEED", VGA_WHITE, colorScreen);
  tft.fillRect(105, 270, 265, 10, VGA_WHITE);
  tft.fillRect(speed + 105, 270, 10, 10, VGA_RED);
  printTFT(415, 270, 2, " " + (String)speed + "  ", VGA_WHITE, colorScreen);
...
{
  printTFT(30, 200, 2, "SOUND", VGA_WHITE, colorScreen);
  tft.fillRect(105, 200, 310, 10, VGA_WHITE);
  tft.fillRect(loudness*10 + 105, 200, 10, 10, VGA_RED);
  printTFT(415, 200, 2, " " + (String)loudness + " ", VGA_WHITE, colorScreen);
...
void Move(String move)
{
  ...
  {
     MotorLeft("FORWARD", 30 * speed / 100);
     MotorRight("FORWARD", speed);
  }
  else if(move == "FORWARD_RIGHT")
  {
     MotorLeft("FORWARD", speed);
     MotorRight("FORWARD", 30 * speed / 100);
  }
  else if(move == "BACKWARD_LEFT")
  {
     MotorLeft("BACKWARD", 30 * speed / 100);
     MotorRight("BACKWARD", speed);
  }
  else if(move == "BACKWARD_RIGHT")
  {
     MotorLeft("BACKWARD", speed);
     MotorRight("BACKWARD", 30 * speed / 100);
  }
