...
{ 
  ...
  /*
  analogWrite(LED, DIASTOLE_BRIGHTNESS);
  analogWrite(MOTOR, DIASTOLE_FREQUENCY);
  _delay_ms(DIASTOLE);
  
  analogWrite(LED, ATRIAL_SYSTOLE_BRIGHTNESS);
  if(vibration)
    analogWrite(MOTOR, ATRIAL_SYSTOLE_FREQUENCY);
  _delay_ms(ATRIAL_SYSTOLE);
  
  analogWrite(LED, DIASTOLE_BRIGHTNESS);
  if(vibration)
    analogWrite(MOTOR, DIASTOLE_FREQUENCY);
  _delay_ms(ATRIAL_DIASTOLE);
  
  analogWrite(LED, VENTRICULAR_SYSTOLE_BRIGHTNESS);
  if(vibration)
    analogWrite(MOTOR, VENTRICULAR_SYSTOLE_FREQUENCY);
  _delay_ms(VENTRICULAR_SYSTOLE);*/
}
