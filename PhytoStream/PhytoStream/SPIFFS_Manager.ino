String getJsonStatus(){
  size_t capacity = JSON_OBJECT_SIZE(8) + 41 + 6 * 3;
  String json; 
  ...
  doc["water_timer_state"] = (byte)isWateringTimerUsed;
...
String graphToJsonForWeb() {
  size_t capacity = JSON_OBJECT_SIZE(2) + 2 * JSON_ARRAY_SIZE(DRAPH_DATA_AMOUNT) + DRAPH_DATA_AMOUNT * 40 + DRAPH_DATA_AMOUNT * 3;
  String json; 
...
String wateringConfigToJson(){
  size_t capacity = JSON_OBJECT_SIZE(7) + 21 + 7 * 3;
  String json; 
  ...
  doc["is_tank_used"] = liquidTank.isUsed();
  doc["empty_depth_mm"] = liquidTank.getEmptyDepthMm();
  doc["full_depth_mm"] = liquidTank.getFullDepthMm();
  doc["is_timer_used"] = isWateringTimerUsed;
  doc["timer_period_s"] = waterTimerPeriod;
...
bool wateringConfigFromJson(String json){
  ...
  if(!error){
    ...
    liquidTank.setState(doc["is_tank_used"]);
    liquidTank.setEmptyDepthMm(doc["empty_depth_mm"]);
    liquidTank.setFullDepthMm(doc["full_depth_mm"]);
    isWateringTimerUsed = doc["is_timer_used"];
    waterTimerPeriod = doc["timer_period_s"];
    waterTimerSeconds = waterTimerPeriod * 3600;
            
    return writeToFlash("/watering_config.txt", json) && waterTimerSecondsSave();   
  }
  if(DEBUG){
    Serial.print("[json error]: ");
    Serial.println(error.c_str());
