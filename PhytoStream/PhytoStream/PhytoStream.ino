...
void setup(){
  ...
  HTTP.on("/get_graph", [](){        
      return HTTP.send(200, "text/plain", graphToJsonForWeb());
  });
  ...
  if(str.length() != 0){
    waterTimerSeconds = atol(str.c_str());
  }
  else{
...
void loop(){
  ...
  if(!resetButtonPressed){
    ...
    if(!isWateringTimerUsed && !wateringManager.isWatering() && (depthPercent > liquidTank.getMinDepthPercent() || !liquidTank.isUsed()) && 
        soilMoisturePercent != -1 && soilMoisturePercent <= soilMoistureSensor.getMinSoilMoisturePercent()){
      wateringManager.autoWatering();
