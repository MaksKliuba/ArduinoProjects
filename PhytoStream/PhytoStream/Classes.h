class TimeManager{
  public:
    ...
    static String getDateNow(time_t t = now(), String dateSeparator = "."){
      if(isTimeUpdated){
        return (String)year(t) + dateSeparator + str(month(t)) + dateSeparator + str(day(t));
      }
      else {
        return "----.--.--";
      }
    ...
    static String getDateTimeNow(time_t t = now(), String dateSeparator = "."){
      return getDateNow(t, dateSeparator) + " " + getTimeNow(t);
    }
...
class Log {
  ...
  public:
    Log(String _path, unsigned long _maxSize = 100000){
      path = _path;
      maxSize = _maxSize;
    }
...
class Filterer {
  ...
  public: 
    double expRunningAverage(double newVal) {
      delta = abs(filVal - newVal);
      delta > 1 ? k = 0.7 : k = 0.1;    
      filVal = newVal * k + filVal * (1 - k);
  ...
  public:
    ...
    int getSoilMoisturePercent(){
      ...
      if(soilMoisture >= MIN_SOIL_MOISTURE){
        return constrain(map(soilMoisture, MIN_SOIL_MOISTURE, MAX_SOIL_MOISTURE, 0, 100), 0, 100);
      }
      else {
        return -1;
      } 
